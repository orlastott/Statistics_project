...
draws %>%
  ggplot() + 
  geom_function(fun = dbeta, args = list(shape1 = e_a, shape2 = e_b)) + 
  geom_histogram(mapping = aes(x = `endurance[50]`, after_stat(density)), 
...
draws %>%
  ggplot() + 
  geom_function(fun = dgamma, args = list(shape = s_shape, rate = s_rate)) + 
  geom_histogram(mapping = aes(x = `speed[50]`, after_stat(density)), alpha = 0.1, 
               binwidth = 0.1) +
  ylab("P.d.f") +
  xlab("S") +
  theme_classic() +
...
mcmc_intervals(draws[c(51,121)]) #Endurance.
mcmc_intervals(draws[c(188,258)]) #Speed.
